#include "Window.h"
#include <windowsx.h>
#include "DirectXApp.h"  // Теперь можем включать, т.к. есть предварительное объявление в .h
...
LRESULT CALLBACK Window::WindowProc(HWND hWnd, UINT message,
    ...
    case WM_ACTIVATE:
        if (LOWORD(wParam) == WA_INACTIVE) {
            // Окно деактивировано - ставим на паузу
            if (window && window->GetDirectXApp()) {
                window->GetDirectXApp()->StopTimer();
            }
        }
        else {
            // Окно активировано - снимаем с паузы
            if (window && window->GetDirectXApp()) {
    ...
    case WM_ENTERSIZEMOVE:
        // Начали изменять размер окна - пауза
        if (window && window->GetDirectXApp()) {
            window->GetDirectXApp()->StopTimer();
        }
        return 0;
    ...
    case WM_EXITSIZEMOVE:
        // Закончили изменять размер окна
        if (window && window->GetDirectXApp()) {
            window->GetDirectXApp()->StartTimer();
